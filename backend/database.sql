...
CREATE TABLE IF NOT EXISTS rim (
  ...
  CONSTRAINT positive_iso_diameter_max_1000 CHECK(iso_diameter BETWEEN 0 AND 1000),
  CONSTRAINT positive_hole_count_max_200 CHECK(hole_count BETWEEN 0 AND 200),
  CONSTRAINT non_negative_offset CHECK(offset_spoke_bed >= 0),
  CONSTRAINT positive_erd CHECK(erd > 0)
);
-- Hub
CREATE TABLE IF NOT EXISTS hub (
  ...
  CONSTRAINT positive_hole_count_max_200 CHECK(hole_count BETWEEN 0 AND 200),
  CONSTRAINT driver_type CHECK(driver IN ('XD', 'Hyperglide 10speed', 'Hyperglide 11speed', 'Hyperglide 7speed','XDR', 'Microspline', 'Uniglide', 'Helicomatic', 'Freewheel ISO', 'Freewheel M30', 'Fixed', 'Flipflop FW/FX', 'Flipflop FW/FW', 'Flipflop FX/FX', 'Proprietary/Other', 'Campagnolo 8speed', 'Campagnolo 9speed')),
  CONSTRAINT brake_type CHECK(brake IN ('6-Bolt', 'Centerlock', 'Drum', 'Roller', 'Coaster', 'Rohloff', '3-Bolt')),
  CONSTRAINT boost_type CHECK(boost IN ('Boost', 'Superboost'))
);
